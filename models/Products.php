...
class Products
{
    ...
    public static function getLatestProducts($count = self::SHOW_BY_DEFAULT)
    {
        ...
        $db = Db::getConnection();
        // Текст запроса к БД
        $sql = 'SELECT id, name, price, is_new FROM car '
    ...
    {
        $limit = Products::SHOW_BY_DEFAULT;
    ...
    public static function getProductsBySearch($text)
    {
        
        $db = Db::getConnection();
        
        $sql = "SELECT id, name FROM car WHERE name  LIKE '%$text%' ";
        $result = $db->prepare($sql);
        $result->setFetchMode(PDO::FETCH_ASSOC);
        $result->execute();
    ...
    public static function getTotalProductsInCategory($categoryId)
    {
        ...
        $db = Db::getConnection();
        // Текст запроса к БД
        $sql = 'SELECT count(id) AS count FROM car WHERE status="1" AND category_id = :category_id';
        // Используется подготовленный запрос
        $result = $db->prepare($sql);
        $result->bindParam(':category_id', $categoryId, PDO::PARAM_INT);
        // Выполнение коменды
    ...
    public static function getProdustsByIds($idsArray)
    {
        ...
        // Текст запроса к БД
        $sql = "SELECT * FROM car WHERE status='1' AND id IN ($idsString)";
        $result = $db->query($sql);
        // Указываем, что хотим получить данные в виде массива
        $result->setFetchMode(PDO::FETCH_ASSOC);
        // Получение и возврат результатов
        ...
        while ($row = $result->fetch()) {
            ...
            $products[$i]['name'] = $row['name'];
            $products[$i]['price'] = $row['price'];
            $products[$i]['avlaibility'] = $row['avlaibility'];
            $i++;
        }
        return $products;
    }
    ...
    public static function getRecommendedProducts()
    {
        ...
        $db = Db::getConnection();
        // Получение и возврат результатов
        $result = $db->query('SELECT id, name, price, is_new FROM car '
    ...
    public static function getProductsList()
    {
        ...
        $db = Db::getConnection();
        // Получение и возврат результатов
        $result = $db->query('SELECT id, name, price, code FROM car ORDER BY id ASC');
        $productsList = array();
        $i = 0;
        while ($row = $result->fetch()) {
            $productsList[$i]['id'] = $row['id'];
    ...
    public static function getModelsList()
    {
                ...
                $result = $db->query('SELECT id, model_name FROM models ORDER BY id ASC');
                ...
                while($row = $result->fetch())
                {
                    $modelsList[$i]['id'] = $row['id'];
                    $modelsList[$i]['model_name'] = $row['model_name'];
                    $i++;
                }
        ...
        public static function getModelsListById($id)
    {
                $db = Db::getConnection();
                
                $result = $db->query("SELECT id, model_name FROM models WHERE id IN ($id)");
     ...
     */
     ...
     public static function updateModelById($id, $options)
    {
        // Соединение с БД
        $db = Db::getConnection();
    ...
    public static function updateProductById($id, $options)
    {
        ...
        $result->bindParam(':avlaibility', $options['avlaibility'], PDO::PARAM_INT);
    ...
    public static function createProduct($options)
    {
        ...
        // Текст запроса к БД
        $sql = 'INSERT INTO car '
                . '(name,model_id, code, price, category_id, brand, avlaibility,'
        ...
        $result = $db->prepare($sql);
        $result->bindParam(':name', $options['name'], PDO::PARAM_STR);
        $result->bindParam(':model_id', $options['model_id'], PDO::PARAM_STR);
        $result->bindParam(':code', $options['code'], PDO::PARAM_STR);
        $result->bindParam(':price', $options['price'], PDO::PARAM_STR);
        $result->bindParam(':category_id', $options['category_id'], PDO::PARAM_INT);
        $result->bindParam(':brand', $options['brand'], PDO::PARAM_STR);
